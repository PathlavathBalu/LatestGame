...
function getElementDetails(elementId) {
  if (elementId != undefined) {
    ...
    let chipCntr = elementId.substr(5) + "cntr";
    let chipId = elementId + "cntr";
    let cardIndex = tileNo.substr(4, 1) - 1;
    let cntrValue = document.getElementById(chipId).innerHTML;
...
function removeChip(chip) {
  ...
  if (element != undefined) {
    if (element.cntrValue > 1) {
      document.getElementById(element.chipId).innerHTML = element.cntrValue - 1;
    } else {
      removeElement(chip);
      removeElement(element.chipId);
    }
...
function reduceBoxAmount(betAmount, tileNo) {
  ...
  document.getElementById(constants.activeBoxes[tileNo]).innerHTML =
    Number(amt) - betAmount;
    totalBetAmt = totalBetAmt-betAmount;
    document.getElementById('totalBet').innerHTML =totalBetAmt;
}
